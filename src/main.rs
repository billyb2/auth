...
use bfsp::auth::{CreateUserRequest, EmailCaveat, LoginRequest};
...
async fn create_user(
        ...
        .await
    {
        ...
        Err(err) => match err.as_database_error().unwrap().kind() {
            ...
            sqlx::error::ErrorKind::Other => todo!("{err:?}"),
            _ => todo!(),
        },
    }
}
...
async fn login(
    ...
    let password: Option<String> = sqlx::query("select password from users where email = ?")
